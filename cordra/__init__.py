""" This is a simple Python library for interacting with
the REST interface of an instance of Cordra.
"""
import requests
import json
...
class CordraObject:
    ...
    def read(cls,
        ...
        Finds the Cordra object by its handle and returns its JSON.

        Args:
            host:
                The address of the instance of Cordra.
                Usually https://localhost:8443.
            obj_id: Object's Cordra ID (including prefix).
            username: Cordra's username.
            password: Cordra's password.
            token (cordra.Token):
                Cordra's authentication token.
                If provided, no username and no password needed.
            verify (bool): SSL verification.
            json_pointer: Optional name of an item in the object.
            json_filter: Optional filter to items in the object.
            full (bool): Return meta-data in addition to object data.

        Returns:
            A dictionary that is the JSON representation of the object.
    ...
    def read_payload_info(cls,
        ...
        Finds the Cordra object by its handle and
        returns the names of its payloads.
    ...
    def read_payload(cls,
        ...
        Finds the Cordra object by its handle, then
        finds the specified payload and returns it as a BytesObject.
    ...
    def update(cls,
               ...
               acls=None):
        """Update a Cordra object by identifier.

        Finds the Cordra object by its handle and
        updates the specified information.
        See ``CordraObject.create()`` for details
        on ``payloads`` and ``acls``.
        ...
        Args:
            ...
            obj_id: Object's Cordra ID (including prefix).
            obj_json (dict):
                This will entirely replace the current JSON
                of the object. To update just one entry,
                use ``json_pointer``.
            json_pointer: Optional name of an item in the object to update.
            obj_type: Object's Type in Cordra.
            dryRun (bool):
                If True: run but don't actually add the item.
                Default is False.

            payload_to_delete: Optional name of a payload to delete.
            username: Cordra's username.
            password: Cordra's password.
            token (cordra.Token):
                Cordra's authentication token.
    ...
    def delete(cls,
               ...
               token=None,
               verify=None):
        """Delete a Cordra object by identifier.
        ...
        Args:
            host:
                ...
                Usually https://localhost:8443.
            obj_id: Object's Cordra ID (including prefix).
            json_pointer: Optional name of an item in the object to delete.
            username: Cordra's username.
            password: Cordra's password.
            token (cordra.Token):
                Cordra's authentication token.
    ...
    def find(cls,
        ...
        Returns Cordra objects that are results of
        the query.
        ...
        Args:
            ...
            token (cordra.Token):
                ...
                If provided, no username and no password needed.
            verify (bool): SSL verification.
            ids (bool): Return only objects' ids?
            json_filter: Optional filter to items in the object.
            full (bool): Return meta-data in addition to object data.
            page_num (int): The desired results page number. Starts at 0.
            page_size (int):
                If negative: no limit.
                If 0: no results are returned, only the size (number of hits).
